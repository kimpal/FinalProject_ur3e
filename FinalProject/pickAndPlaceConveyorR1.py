...
placeObjectOnConveyor = 0.2, 0.3, 0.1, 2.24, 2.2, 0
...
def pickObject():
    global x, y, lastx, lasty, objectCount, placeObjectOnConveyor, PickVia, placeVia
    objectCount+= 1
    lastx = x
    lasty = y
    ...
    move(rob, placeObjectOnConveyor, True)
    rob.send_program(rq_open())
    time.sleep(0.2)
    # position before picing New bloc to avoid colision
